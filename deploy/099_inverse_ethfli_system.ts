...
import {
  ...
  EXCHANGE_NAMES,
  EXCHANGE_SETTINGS,
} from "@deployments/constants/099_inverse_ethfli_system";
...
const func: DeployFunction = trackFinishedStage(CURRENT_STAGE, async function (hre: HRE) {
  ...
  await addExtension(CONTRACT_NAMES.BASE_MANAGER_NAME, CONTRACT_NAMES.FLEXIBLE_LEVERAGE_EXTENSION_NAME);
  ...
  async function polyFillForDevelopment(): Promise<void> {
    if (await findDependency(I_ETHFLI) === "") {
      await writeContractAndTransactionToOutputs(I_ETHFLI, await getRandomAddress(), EMPTY_BYTES, "Created Mock I_ETHFLI");
    }
  ...
  async function deployFlexibleLeverageStrategyExtension(): Promise<void> {
    const checkFlexibleLeverageExtensionAddress = await getContractAddress(CONTRACT_NAMES.FLEXIBLE_LEVERAGE_EXTENSION_NAME);
    if (checkFlexibleLeverageExtensionAddress === "") {
      ...
      const contractSettings: ContractSettings = {
        setToken: await findDependency(I_ETHFLI),
        leverageModule: await findDependency(COMPOUND_LEVERAGE_MODULE),
        comptroller: await findDependency(COMPOUND_COMPTROLLER),
        targetCollateralCToken: await findDependency(C_USDC),
        targetBorrowCToken: await findDependency(C_ETH),
        collateralAsset: await findDependency(USDC),
        borrowAsset: await findDependency(WETH),
        collateralPriceOracle: await findDependency(CHAINLINK_USDC),
        borrowPriceOracle: await findDependency(CHAINLINK_ETH),
      ...
      flexibleLeverageDeploy.receipt && await saveContractDeployment({
        name: CONTRACT_NAMES.FLEXIBLE_LEVERAGE_EXTENSION_NAME,
        contractAddress: flexibleLeverageDeploy.address,
        id: flexibleLeverageDeploy.receipt.transactionHash,
        description: "Deployed InverseETHFlexibleLeverageStrategyExtension",
        constructorArgs,
      });
    }
  }
