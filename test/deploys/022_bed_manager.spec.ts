...
describe("BED: Manager System", () => {
  ...
  before(async () => {
    ...
    const deployedFeeExtension = await await getContractAddress("StreamingFeeSplitExtension - BED");
  ...
  describe("BaseManager", async () => {
    it("should have the correct SetToken address", async () => {
      ...
      expect(setToken).to.eq(await findDependency("BED"));
    });
  ...
  describe("GIMExtension", async () => {
    it("should have the correct manager address", async () => {
      ...
      expect(manager).to.eq(await getContractAddress("BaseManager - BED"));
    });
